          ...
          {!success && (
            <div style={{ padding: '32px 32px 64px', maxWidth: 1080, width: '100%', margin: '0 auto' }}>
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap', marginBottom: 24 }}>
                <div>
                  <h1 style={{ fontFamily: "'Instrument Serif',serif", fontWeight: 400, fontSize: 38, lineHeight: 1, margin: '0 0 8px' }}>Choose your plan</h1>
                  <p style={{ fontSize: 15, color: '#5A544A', margin: 0 }}>Pick the plan that fits your search — change or cancel anytime.</p>
                </div>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: "'JetBrains Mono',monospace", fontSize: 11, fontWeight: 600, color: '#9A6A2E', background: '#FBF1E2', border: '1px solid #EAD9BE', padding: '6px 12px', borderRadius: 999 }}>
              ...
              <div style={{ display: 'flex', gap: 32, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 320 }}>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#F1ECE0', border: '1px solid #E1D9C9', borderRadius: 999, padding: 5, marginBottom: 18 }}>
                    ...
                    <button onClick={() => setAnnual(true)} style={{ background: annualBg, color: annualColor, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, padding: '8px 18px', borderRadius: 999, display: 'flex', alignItems: 'center', gap: 8 }}>
                      Annual <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, background: '#1FA463', color: '#0C2C1C', padding: '2px 7px', borderRadius: 999 }}>−33%</span>
                    </button>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
                    {plans.map((p) => (
                      ...
                      >
                        {p.popular && (
                          <span style={{ position: 'absolute', top: -10, left: 20, fontFamily: "'JetBrains Mono',monospace", fontSize: 9.5, fontWeight: 600, letterSpacing: '0.06em', color: '#0C2C1C', background: '#1FA463', padding: '3px 10px', borderRadius: 999 }}>MOST POPULAR</span>
                        )}
                        ...
                        <span style={{ textAlign: 'right', flexShrink: 0 }}>
                          <span style={{ display: 'block' }}>
                            <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 22, fontWeight: 600, color: p.nameColor }}>{p.price}</span>
                            <span style={{ fontSize: 12, color: p.taglineColor }}>{p.per}</span>
                          </span>
                          <span style={{ display: 'block', fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: p.billColor }}>{p.billNote}</span>
                        </span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div style={{ background: '#EAF6EE', border: '1px solid #CDE9D6', borderRadius: 16, padding: 22 }}>
                    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#157A49', marginBottom: 14 }}>{delta.label}</div>
                ...
                <div style={{ width: 380, flexShrink: 0, position: 'sticky', top: 84, display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ background: '#FFFEFB', border: '1px solid #E6DECF', borderRadius: 16, padding: 22 }}>
                    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#9A9286', marginBottom: 16 }}>Order summary</div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
                      ...
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 15, fontWeight: 600 }}>{priceLine}</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 9, padding: '14px 0', borderTop: '1px solid #F2ECE0', borderBottom: '1px solid #F2ECE0', marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13 }}>{subtotal}</span>
                      </div>
                      {hasDiscount && (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: 13, color: '#157A49' }}>Annual discount</span>
                          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, color: '#157A49' }}>−{money(discountAmt)}</span>
                        </div>
                      )}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 13, color: '#5A544A' }}>Tax (est. 8.5%)</span>
                        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13 }}>{money(tax)}</span>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 14, fontWeight: 700 }}>{totalLabel}</span>
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 24, fontWeight: 600, color: '#1B1A16' }}>{money(total)}</span>
                    </div>
                  </div>

                  {/* PAYMENT */}
                  <div style={{ background: '#FFFEFB', border: '1px solid #E6DECF', borderRadius: 16, padding: 22 }}>
                    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#9A9286', marginBottom: 16 }}>Payment details</div>
          ...
          {success && (
            <div style={{ padding: '48px 32px 64px', maxWidth: 600, width: '100%', margin: '0 auto' }}>
              <div style={{ background: '#FFFEFB', border: '1px solid #E6DECF', borderRadius: 20, padding: '48px 36px', textAlign: 'center', animation: 'rbpop 0.35s ease' }}>
                ...
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#157A49', marginBottom: 12 }}>Upgrade complete</div>
                <h1 style={{ fontFamily: "'Instrument Serif',serif", fontWeight: 400, fontSize: 36, lineHeight: 1.05, margin: '0 0 12px' }}>You’re on {planName}.</h1>

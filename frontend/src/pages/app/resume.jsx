 ...
 * and changeable at any point during it. Starting again provisions a new
 * server-routed sandbox; changing the look does not.
 */
...
export default function AppResume() {
  ...
  const acceptSession = useCallback((response) => {
    ...
    if (
      ...
      window.sessionStorage.getItem(ACTIVE_SESSION_KEY) === current.id
    ) {
  ...
  const start = async (requestedSourceSessionId) => {
    ...
    try {
      const next = await startHarnessSession({
        ...
        ...(composedPastedJobDescription()
          ? { jobDescription: composedPastedJobDescription() }
          : {}),
      ...
      <style jsx global>{`
        ...
        #jbres select:focus {
          ...
          box-shadow: 0 0 0 3px
            color-mix(in srgb, var(--jb-v3-accent) 14%, transparent);
        }
        /*
    ...
    return session.conversation.map((message) => {
      const turn =
        message.revision == null
          ? null
          : (session.turns || []).find(
              (candidate) => candidate.revision === message.revision,
            );
      return {
  ...
  return (
    ...
    >
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: "0 0 22px" }}>
        {(unknown ? ["name", "email", "linkedin", "location"] : missing).map(
          (f) => (
            <li
              key={f}
            ...
            >
              <span
                aria-hidden="true"
                style={{ color: "#b4232a", fontWeight: 700 }}
              >
                *
              </span>
              {FIELD_LABELS[f] || f}
...
/**
 * Model and effort as two controls over capability metadata.
 *
 * The backend hands over only the model capabilities the caller's tier permits,
 * so both dropdowns are derived from metadata rather than from a table here.
 * Provider aliases and the execution runtime never enter the UI contract.
 *
...
function ModelAndEffort({
  ...
  const selected =
    models.find((item) => item.model === selectedModel) || models[0];
  ...
  const pickModel = (model) => {
    ...
    setEffort(next.efforts.includes(effort) ? effort : next.efforts[0]);
  };
    ...
    <>
      ...
      >
          ...
          >
            ...
            {p.livePhase && (
              <div
                ...
                style={{ animation: "jbRise .2s ease" }}
              >
                <div
                  style={{
                    display: "flex",
        ...
        >
          ...
          >
            <summary
              style={{ ...label, padding: "11px 18px", cursor: "pointer" }}
            >
              Revision history (
              {session.revisionCount ?? session.turns?.length ?? 0})
            </summary>
            <div
              style={{ maxHeight: 220, overflow: "auto", padding: "0 18px" }}
            >
              {!(session.turns || []).length && (
                <p style={{ fontSize: 13, color: T.fg3 }}>
                  Your revisions will appear after the first generation.
                </p>
              )}
                ...
                >
                  ...
                  >
                    <div style={{ color: T.fg }}>
                      Revision {turn.revision}
                      {turn.revision === session.revision ? " · current" : ""}
                    </div>
                    {turn.summary && (
                      <div style={{ color: T.fg2 }}>{turn.summary}</div>
                    )}
                    {turn.instruction && (
                    ...
                    <div style={{ color: T.fg3 }}>
                      ...
                      {turn.compiled ? "passing" : "failing"}
                    </div>
                  </div>
                  <button
                    aria-label={`Restore revision ${turn.revision}`}
                    disabled={p.busy}
                    onClick={() => p.restoreRevision(turn.revision)}
                    style={ghostBtn}
                  >
                    Restore
                  </button>
...
function AtsPanel({
  ...
  const missingContext =
    session.revision > 0 && !jobDescription?.trim() && !jobUrl?.trim();
    ...
    >
      ...
      >
        <div style={{ flex: 1, minWidth: 260 }}>
          ...
          {visibleWarning && (
            <p
              data-testid="ats-job-warning"
              style={{ margin: "8px 0 0", color: "#9a6a2e", fontSize: 12.5 }}
            >
              {visibleWarning}
            </p>
          )}
          {session.status !== "active" && (
            <p style={{ margin: "8px 0 0", color: T.fg3, fontSize: 12.5 }}>
              Saved results remain available. Continue from here to run it
              again.
            </p>
          )}
          {stale && (
            <p
              data-testid="ats-stale"
              style={{
                margin: "8px 0 0",
                color: "#9a6a2e",
                fontSize: 12.5,
          ...
          <div style={{ textAlign: "right", minWidth: 90 }}>
            ...
            >
              <span style={{ fontSize: 42 }}>
                {Math.round(result.semanticMatch)}
              </span>
            ...
            <div
              ...
              style={{
                ...
                fontSize: 12.5,
                fontWeight: 600,
                color: goodToSubmit ? "#4f9b73" : "#9a6a2e",
              }}
            >
              {goodToSubmit ? "Good to submit" : "Improve before submitting"}
            </div>
            <div data-testid="ats-revision" style={{ ...label, marginTop: 5 }}>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12.5, color: T.fg2, lineHeight: 1.7 }}>
              <div>
                Keywords · {Math.round(result.subScores?.keywordMatch ?? 0)}
              </div>
              <div>
                Skills · {Math.round(result.subScores?.skillsCoverage ?? 0)}
              </div>
              <div>
                Sections ·{" "}
                {Math.round(result.subScores?.sectionCompleteness ?? 0)}
              </div>
            </div>
          </div>
        ...
        {!you && message.revision != null && (
          <div
            style={{
              ...
              fontSize: 10,
              marginTop: 7,
              color: message.compiled ? T.fg3 : "#b4232a",
            }}
          >
            revision {message.revision} · build{" "}
            {message.compiled ? "passing" : "failing"}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {session.canRevert && (
          <span
            data-testid="look-revert-available"
            style={{ ...label, fontSize: 9.5, color: T.accent }}
          >
        ...
        >
          ...
          >
            ...
            <button
              ...
              style={{
                ...ghostBtn,
                opacity: busy || !session.canRevert ? 0.45 : 1,
              }}
            >
            ...
            <span
              data-testid="look-status"
              style={{ fontSize: 12.5, color: T.fg3 }}
            >
              {dirty
                ? templateChanged

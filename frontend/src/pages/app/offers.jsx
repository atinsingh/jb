...
export default function AppOffers() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (alive) {
          setError(err || new Error('Could not load offers'));
          setData([]);
        }
      } finally {
  ...
  const { offerCount, offers, headers, rows } = useMemo(
    () => (hasOffers ? buildVals(data, selected) : { offerCount: 0, offers: [], headers: [], rows: [] }),
    [data, selected, hasOffers]
  );

  const earliestDeadline = useMemo(
    () => (data.length ? Math.min(...data.map((o) => o.deadlineDays)) : 0),
    [data]
  );
      ...
      <div id="jbapp" style={{ display: 'flex', minHeight: '100vh', background: '#F7F3EA', fontFamily: 'var(--jb-font-sans)', color: '#1B1A16' }}>
        ...
        <main style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <header style={{ position: 'sticky', top: 0, zIndex: 20, display: 'flex', alignItems: 'center', gap: 20, padding: '15px 32px', background: 'rgba(247,243,234,0.85)', backdropFilter: 'blur(10px)', borderBottom: '1px solid #E7E0D2' }}>
            <div style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 11.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9A9286' }}>Workspace / Offers</div>
            <div style={{ flex: 1 }} />
            {!loading && !error && hasOffers && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontFamily: 'var(--jb-font-mono)', fontSize: 11.5, color: '#C9622E' }}>
          ...
          <div style={{ padding: '30px 32px 56px', width: '100%' }}>
            ...
            <div style={{ marginBottom: 24 }}>
              <h1 style={{ fontFamily: 'var(--jb-font-display)', fontWeight: 400, fontSize: 40, lineHeight: 1, letterSpacing: '-0.01em', margin: '0 0 8px' }}>Offers</h1>
              <p style={{ fontSize: 15.5, color: '#5A544A', margin: 0 }}>
                {loading
                  ? 'Loading your offers…'
                  : <><b style={{ color: '#1B1A16' }}>{offerCount} active offer{offerCount === 1 ? '' : 's'}</b> on the table · compare and decide before they expire.</>}
              </p>
            </div>
            ...
            {!loading && !error && !hasOffers && (
              <div style={{ background: '#FFFEFB', border: '1px solid #E6DECF', borderRadius: 18 }}>
                <EmptyState
                  icon="✦"
                  title="No offers yet"
                  hint="When an employer extends an offer, it’ll appear here so you can compare comp side by side."
                />
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 14 }}>
              {offers.map((o) => (
                ...
                >
                  {o.best && (
                    <span style={{ position: 'absolute', top: -11, left: 22, fontFamily: 'var(--jb-font-mono)', fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', color: '#0C2C1C', background: '#1FA463', padding: '4px 11px', borderRadius: 999 }}>★ BEST TOTAL</span>
                  )}
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, marginBottom: 3 }}>
                    <span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 38, fontWeight: 600, lineHeight: 1, color: '#1B1A16' }}>{o.totalFmt}</span>
                  </div>
                  <div style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 11, color: '#8A8378', marginBottom: 18 }}>/ yr total comp · +{o.signonFmt} sign-on</div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '14px 0', borderTop: '1px solid #F2ECE0', borderBottom: '1px solid #F2ECE0', marginBottom: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ fontSize: 13, color: '#8A8378' }}>Base</span><span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 13, fontWeight: 600, color: '#1B1A16' }}>{o.baseFmt}</span></div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ fontSize: 13, color: '#8A8378' }}>Equity / yr</span><span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 13, fontWeight: 600, color: '#1B1A16' }}>{o.equityFmt}</span></div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ fontSize: 13, color: '#8A8378' }}>Bonus (target)</span><span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 13, fontWeight: 600, color: '#1B1A16' }}>{o.bonusFmt}</span></div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 18 }}>
                    ...
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, alignSelf: 'flex-start', fontFamily: 'var(--jb-font-mono)', fontSize: 11, fontWeight: 600, color: o.deadlineColor, background: o.deadlineBg, border: `1px solid ${o.deadlineBorder}`, padding: '4px 10px', borderRadius: 999 }}>⏳ {o.deadlineDays} days to respond</div>
                  </div>
            ...
            >
              ...
              <span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 12, fontWeight: 600, color: '#5BD08C', flexShrink: 0 }}>Start a chat →</span>
            </Link>
            ...
            <div style={{ background: '#FFFEFB', border: '1px solid #E6DECF', borderRadius: 18, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #F2ECE0' }}>
                ...
                <span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#5BD08C' }}>Mint = best in row</span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr 1fr 1fr', alignItems: 'center', padding: '14px 22px', borderBottom: '1px solid #F2ECE0' }}>
                <span style={{ fontFamily: 'var(--jb-font-mono)', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#9A9286' }}>Component</span>
                {headers.map((h, i) => (
              ...
              {rows.map((r, ri) => (
                <div key={ri} style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr 1fr 1fr', alignItems: 'center', padding: '13px 22px', borderBottom: `1px solid ${r.divider}`, background: r.rowBg }}>
                  ...
                  {r.cells.map((c, ci) => (
                    <span key={ci} style={{ justifySelf: 'start', fontFamily: 'var(--jb-font-mono)', fontSize: c.size, fontWeight: c.weight, color: c.color, background: c.bg, border: `1px solid ${c.cellBorder}`, padding: '5px 11px', borderRadius: 8 }}>{c.text}</span>
                  ))}
                </div>
              ))}
            </div>
